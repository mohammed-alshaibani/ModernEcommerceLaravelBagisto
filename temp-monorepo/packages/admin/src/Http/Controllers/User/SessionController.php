...
class SessionController extends Controller
{
    ...
    public function store()
    {
        ...
        if (! bouncer()->hasPermission('dashboard')) {
            ...
            // If user has no role, redirect to unauthorized page or log them out
            if (!$user->role) {
                auth()->guard('admin')->logout();
                session()->flash('error', 'Your account is not assigned a role. Please contact your administrator.');
                return redirect()->route('admin.session.create');
            }
            ...
            foreach ($permissions as $permission) {
                if (bouncer()->hasPermission($permission)) {
                    ...
                    if ($permissionDetails && isset($permissionDetails['route'])) {
